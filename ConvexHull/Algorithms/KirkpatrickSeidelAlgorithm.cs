...
public class KirkpatrickSeidelAlgorithm : IConvexHullAlgorithm
{
    ...
    private IEnumerable<Vector3> ComputeUpperHull(List<Vector3> input)
    {
        ...
        var computeUpperHull = Connect(min, max, T).ToList();
        return computeUpperHull;
    }
    ...
    private IEnumerable<Vector3> Connect(Vector3 min, Vector3 max, List<Vector3> S)
    {
        ...
        var S_left = new List<Vector3>
            { i };
        S_left.AddRange(S.Where(p => p.X < i.X));

        var S_right = new List<Vector3>
            { j };
        S_right.AddRange(S.Where(p => p.X > j.X));
        ...
        {
            output.AddRange(Connect(min, i, S_left));
        }
        ...
        {
            output.AddRange(Connect(j, max, S_right));
        }
